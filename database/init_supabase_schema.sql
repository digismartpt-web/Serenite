...
CREATE TABLE IF NOT EXISTS users (
  ...
  role                    VARCHAR(20) NOT NULL CHECK (role IN ('parent','child','solo')),
  parent_type             VARCHAR(20) CHECK (parent_type IN ('papa','maman','beau-pere','belle-mere')),
  status                  VARCHAR(20) CHECK (status IN ('separated','divorced')),
...
CREATE TABLE IF NOT EXISTS families (
  ...
  status            VARCHAR(20) DEFAULT 'pending' CHECK (status IN ('pending','active','solo')),
  name              VARCHAR(200),
  created_at        TIMESTAMPTZ DEFAULT NOW()
);
...
SELECT f.id AS family_id, f.name AS family_name, f.status AS family_status,
  pa.id AS parent_a_id, pa.first_name AS parent_a_first_name, pa.last_name AS parent_a_last_name, pa.email AS parent_a_email, pa.parent_type AS parent_a_type, pa.calendar_color AS parent_a_color,
  pb.id AS parent_b_id, pb.first_name AS parent_b_first_name, pb.last_name AS parent_b_last_name, pb.email AS parent_b_email, pb.parent_type AS parent_b_type, pb.calendar_color AS parent_b_color,
  f.created_at
FROM families f LEFT JOIN users pa ON pa.id = f.parent_a_id LEFT JOIN users pb ON pb.id = f.parent_b_id;
...
CREATE TABLE IF NOT EXISTS events (
  ...
  category    VARCHAR(20) DEFAULT 'autre' CHECK (category IN ('visite','vacances','scolaire','medical','activite','autre')),
  color       VARCHAR(7),
  created_at  TIMESTAMPTZ DEFAULT NOW(),
  updated_at  TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS exchange_requests (
  ...
  reason        TEXT,
  proposed_date DATE,
  status        VARCHAR(20) DEFAULT 'pending' CHECK (status IN ('pending','accepted','refused','cancelled')),
...
CREATE TABLE IF NOT EXISTS expenses (
  ...
  category     VARCHAR(30) DEFAULT 'autre' CHECK (category IN ('garde','activite','sante','scolarite','vetement','alimentation','loisir','autre')),
